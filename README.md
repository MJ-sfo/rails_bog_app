Michael's thoughts from working on this durring the weekend:
Gist on Rails

When http://localhost:3000/ gave error that bootstrap wasn’t showing on index page, i suspected the could be wrong. On ‘app/assets/stylesheets/application.css’  Instructions gave
  *= require bootstrap-3.3.7.min
  *= require_tree .
  *= require_self
To be commented out on apps/assets/stylesheets/applications.css, as did the solutions code.  But how would the index page read the bootstrap library if commented out? I found the page that had ‘require bootstrap’, tried with lines un-commented, and http://localhost:3000/  worked fine.


3. Set up the view for the new creature form
Update new route for new creature, update controller, create new form with new.html.erb, on website get error:  SyntaxError in CreaturesController#new.  
syntax error, unexpected ':', expecting keyword_end tput_buffer.append= form_for:
So i look at every colon to find a mistake.  I notice I have form_for: creature , instead of instruction’s form_for :creature , so i change the colon and get an error: for line three.  Looking at new.html.erb, i notice line three and four is a different color than line three, so change line 3 and 4  from f: from colon to dot.

On try 2, I try start with adding one entry, but first have to start rails console

LoveHaightMacs4:bog_app macster$ rails c
/Users/macster/.rvm/gems/ruby-2.4.0/gems/activesupport-5.1.1/lib/active_support/dependencies.rb:286:in `load': /Users/macster/wdi/week8/bog2/bog_app/config/routes.rb:4: syntax error, unexpected tSTRING_BEG, expecting keyword_do or '{' or '('
 , to: "creatures@index", as "creatures"
I see there is a problem in routes.rb - i never figured out, so started again by cloning entire project to bog3

Finally realize i have to create folders assets/stylesheets
Had trouble running curl for bootstrap - ended up copying files that i did in first round
Running ‘rails g controller creatures’ and ‘rails g model creature name description’ didn’t seem to work - never figured out why.  Is it reading something i did in first round, even though i re-cloned to new directory?

Fork/clone to a directory, then this process makes subdirectory -  push up from top folder, but doesn’t seem to include changes in sub-folder
